...
public class CreUserAccountServiceImpl implements CreUserAccountService {
	...
	public BackResult<Boolean> rechargeOrRefunds(TrdOrderDomain trdOrderDomain) {
		...
		switch (trdOrderDomain.getType()) {
			...
			default:
				...
				result.setResultMsg("用户ID为"+trdOrderDomain.getCreUserId()+"无法解析的数据类型：【" + trdOrderDomain.getType() + "】");
				break;
		}
	...
	public BackResult<Boolean> recharge(TrdOrderDomain trdOrderDomain,Integer creUserId) {
		logger.info("用户ID：【" + trdOrderDomain.getCreUserId() + "】执行充值，充值金额：【" + trdOrderDomain.getMoney() +"】，充值条数；【" + trdOrderDomain.getNumber() + "】");
		...
		try {
			...
			logger.error("用户ID：【" + trdOrderDomain.getCreUserId() + "】执行充值，充值金额：【" + trdOrderDomain.getMoney() +"】，充值条数；【" + trdOrderDomain.getNumber() + "】，发生系统异常：" + e.getMessage());
			e.printStackTrace();
			result.setResultCode(ResultCode.RESULT_FAILED);
	...
	public BackResult<Boolean> refunds(TrdOrderDomain trdOrderDomain,Integer creUserId) {
		logger.info("用户ID：【" + trdOrderDomain.getCreUserId() + "】执行退款，退款金额：【" + trdOrderDomain.getMoney() +"】，退款条数；【" + trdOrderDomain.getNumber() + "】");
